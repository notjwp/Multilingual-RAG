...
from 0.669 to **0.852 retention** (full corpus, 150 queries / 20,240 docs) and invalidated several
published numbers. See M19.
...
| — | Eval romanization fix | ✅ **DONE** — queries were generated by the library under test; now human-written (Dakshina). Shipped romanized retention 0.669 → **0.852** (full corpus); older Indic numbers superseded |
| — | M19 close-out (baseline · kn/te · judges) | ✅ **DONE** — full-corpus baseline + reinstated 0.80 gate; kn/te lexicons built but coverage too low, numbers stay directional; no reachable LLM judge clears the bar |
...
- ✅ Measured (XQuAD-hi, 10k distractors, 150 queries, google, recall@5): native **0.947** ·
  ...
  configuration now scores **0.852** shipped retention on the full corpus. See M19 close-out.
  Romanized recovers
...
- ✅ Validation: no ready romanized kn/te corpus (IndicQA-romanized lacks them, FLORES-plus gated,
  ...
  **Wikipedia** (native kn/te sentences) + the `indic_transliteration` romanizer;
  ...
  ⚠️ **Superseded, and not correctable today** — these used the same rigged `romanize()` as the hi
  numbers. kn/te human lexicons now exist (Dakshina) but cover only 42.6% / 43.6% of this corpus's
  vocabulary against a 50% bar, so the bias is not meaningfully removed. Treat as directional only.
...
`agent repaired: 0/60`. ⚠️ **This is the SAMPLED run** — 60 queries / 3,240 docs. Superseded by
the full-corpus baseline in the close-out below, which scores **0.852**: more distractors
compete for the same top-5 slots, so a shortened run reads high. Kept here because the
0.917-vs-0.852 gap is the evidence for gating only full runs.
...
### Close-out — the three deferred items ✅

**1. Full-corpus baseline.** 150 queries / **20,240 documents** / google / k=5, human-written
romanized queries — `data/eval/reports/hi-full-baseline.json`:
...
Human-romanization coverage 90.4%, detection 98.0%, repair fired 0/150.

**0.852 is the baseline that replaces 0.669.** The 3,240-doc sample had said 0.917 — sampling
inflates retention, exactly as flagged, which is why the reinstated acceptance bar (**0.80**,
~0.05 of headroom) **only scores full-corpus runs** and prints "no gate" otherwise. `--report`
now writes metrics *and* config to JSON, so a retention figure can never again be quoted without
the corpus size that produced it.

**2. kn/te lexicons — built, and they don't fix the numbers.** I had recorded this as blocked on a
lexicon that didn't exist; that was wrong, Dakshina covers all twelve languages. But coverage of
the Wikipedia-sentence vocabulary is only **kn 42.6% / te 43.6%** against a 50% bar (hi: 90.4%),
because kn/te have no parallel corpus to supplement Dakshina. The majority of each query is still
sanscript output, so **`kn 0.963 / te 0.975` stay directional** — the lexicons are kept (43% beats
0%, and the fallback share is printed every run) but they do not earn a corrected figure. The
builder prints that verdict itself.

**3. No reachable LLM judge clears the bar.** `scripts/eval_grader.py`, 20 queries each:
...
| gemma / mistral / nemotron / qwen | — | — | 404 — advertised by `models.list()`, not on this key |

Nine times the parameters bought six points. That is the evidence for `score-threshold` as the
default. Two process notes worth keeping: a **one-sided bar would have adopted a broken judge** —
`LlmRelevanceGrader` fails open, so a model returning 404s posted a perfect 0% false-alarm rate
until the catch-rate half of the bar caught it; and I wasted a run on a guessed model id before
probing, which is the same "don't build on an unvalidated measurement" mistake in miniature.

**The repair cycle, observed live** (chat with no documents, so retrieval is genuinely empty —
the one signal the free grader is certain of):

```
event: step  retrieve:1  "Searching your documents"        · 0 passages found
event: step  repair:1    "Didn't find much — trying again" · rephrasing the search
event: step  retrieve:2  "Searching again"                 · 0 passages found
event: step  generate    "Writing the answer"
event: done  citations=0
→ "Apne documenton mein, 'bharat ki rajdhani kya hai' ki jaankari nahin payi gayi hai."
```

Grade → repair → retry → `generate_no_context`, refusing honestly with zero citations. This is the
cycle **refusing**, not rescuing; no rescue transcript exists, and with the 0.0 floor none can on
this corpus.

🐛 **That transcript found a real bug.** The first capture answered in **Albanian**. `langdetect`
labels `bharat ki rajdhani kya hai` as *Swahili*, and `resolve_answer_language` trusted it over
`route_language`, which had already identified Hindi with the purpose-built 98%-accurate detector.
The normal path masks it — Devanagari passages in the prompt make the model mirror them regardless
— so only an empty retrieval exposed it. `RagNodes._response_language` now prefers the router's
