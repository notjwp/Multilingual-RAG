...
# Derived from the full-corpus baseline: 150 queries / 20,240 docs / google / word-list detector
# measured shipped retention 0.852 (data/eval/reports/hi-full-baseline.json, 2026-08-16). The bar
# sits ~0.05 below so ordinary run-to-run variance passes but a real regression fails.
#
# It replaces the historical 0.747, which was derived when the harness generated its queries with
# the same library as the `rule-based` adapter under test — see evaluation/romanization.py. Do not
# compare the two numbers.
ACCEPTANCE_BAR = 0.80
# Only a full-corpus run may be gated: sampling fewer distractors inflates retention (a 3240-doc
# sample scored 0.917 where the full 20,240 scored 0.852).
ACCEPTANCE_MIN_QUERIES = 150
...
def write_report(
    ...
    *,
    ...
    n_detected: int,
    n_repaired: int,
    roman_stats: Counter[str],
) -> None:
    """Persist the run as JSON. A full-corpus run takes ~45 minutes; stdout alone is too fragile.

    Records the config alongside the metrics, because a retention figure means nothing without the
    corpus size, provider and grader that produced it — that ambiguity is exactly what made the
    historical 0.669/0.747 numbers unusable.
    """
    ...
    report = {
        "config": {
            ...
            "queries": n_queries,
            "documents": n_docs,
            "distractor_cap": args.distractor_cap,
            "transliteration_provider": args.provider,
            "transliteration_detector": settings.transliteration_detector,
            "relevance_grader": settings.relevance_grader,
            "relevance_score_threshold": settings.relevance_score_threshold,
            "agent_max_repairs": settings.agent_max_repairs,
        },
        "romanization": {
            "human_words": roman_stats["hit"],
            "fallback_words": roman_stats["oov"],
            "human_share": (roman_stats["hit"] / words) if words else None,
        },
        "detection_rate": n_detected / n_queries if n_queries else None,
        "repair_rate": n_repaired / n_queries if n_queries else None,
        "metrics": {
            cond: {
                f"recall_at_{args.k}": _mean(scores[cond]["recall"]),
                "mrr": _mean(scores[cond]["rr"]),
                "retention": _mean(scores[cond]["recall"]) / native_recall,
            }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Write the run as JSON here (config + metrics). Use it for any long run.",
    )
    ...
    print(f"\nshipped retention: {shipped_ret:.3f}")
    if args.sample >= ACCEPTANCE_MIN_QUERIES and args.distractor_cap is None:
        verdict = "PASS" if shipped_ret >= ACCEPTANCE_BAR else "FAIL"
        print(f"Acceptance (>= {ACCEPTANCE_BAR}): {verdict} ({shipped_ret:.3f})")
    else:
        # A sampled run inflates retention — fewer distractors compete for the same top-k slots.
        # The full run measured 0.852 where a 3240-doc sample measured 0.917, so gating a sample
        # against the full-corpus bar would be meaningless in the flattering direction.
        print(f"  (sampled run — no gate. Full corpus, {ACCEPTANCE_MIN_QUERIES}+ queries, no "
              f"--distractor-cap is required to score against the {ACCEPTANCE_BAR} bar.)")
    if args.report:
        write_report(
            args.report,
            args=args,
