...
**The repair loop is measured at parity, and both grader defaults are deliberately near-inert.** `RELEVANCE_GRADER=score-threshold` with `RELEVANCE_SCORE_THRESHOLD=0.0` means only an *empty* retrieval is graded weak, so the loop almost never fires. Don't "improve" either without re-running `scripts/eval_romanized.py`: every positive floor tried lost to having no agent at all (best 0.767 vs 0.800) because bge-m3's cosine bands for correct and incorrect retrievals overlap, and the `llm` grader with llama-3.1-8b false-alarms on **81% of correct retrievals**. Generation answers from `best_context` — the best-graded attempt, not the last — so a repair can never make things worse; it is adopted only if it flips weak→relevant, never on a higher score. `agentic` is the fifth condition in `eval_romanized.py`, which drives the real graph with generation stubbed.
...
**Two things that will waste your time if you don't know them.** (1) **No reachable LLM judge is good enough** — llama-3.1-8b false-alarms on 81% of correct retrievals, the 70B on 75%, the 3.3-70B times out, and most catalog ids 404 on a free-tier key. Run `scripts/eval_grader.py --model X` before believing any of them, and note the bar is two-sided: the grader fails open, so a model that 404s posts a perfect 0% false-alarm rate. (2) **`langdetect` calls romanized Hindi "Swahili"** — never trust `RetrievalContext.query_language` for a Latin-script query; `route.target_language` is the authority (`RagNodes._response_language`).

**The default grader lets the model hallucinate on out-of-corpus questions — 61% of the time.** Measured by `scripts/eval_refusal.py`, which is the only harness here that asks *unanswerable* questions (XQuAD can't: every question has an answer, so recall@5 0.852 and a majority hallucination rate coexist happily). `RELEVANCE_GRADER=llm` drops it to 0% but refuses 70% of answerable questions. There is no middle: the llm grader prevents hallucination *by refusing*, and an experiment that kept its judgement while answering from `best_context` anyway brought hallucination straight back (0% → 55%) and was reverted. Don't re-propose that without reading `docs/architecture.md §1.9a`.
