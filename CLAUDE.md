...
**One document path.** Documents go through `DatabaseDocumentIndexingService` (Postgres repositories + Celery, user-scoped). The legacy `DocumentIndexingService` + `DocumentStore` (a JSON file, no user scoping) was removed in Phase D — there is now a single source of truth.
...
**Romanized-Indic queries.** `RetrievalService.retrieve` detects romanized Hindi (`transliteration/detect.py::is_romanized_indic`) and, when detected, transliterates the query to Devanagari before embedding, so it matches the native-script index. Plain English is left on the raw path. Detection, not "search both and merge" — the eval proved fusion loses (the raw romanized search is irreducible noise). Provider selected by `TRANSLITERATION_PROVIDER` (default `google`/googletrans with a local rule-based fallback). `TRANSLITERATION_DETECTOR` picks the detector and, with it, which languages are supported: `word-list` (default — a distinctly-Hindi function-word check, ~98% recall/0 FP, fast/local, **Hindi only**), `muril` (opt-in — a frozen `google/muril-base-cased` feature extractor + a committed LogisticRegression head, `transliteration/muril.py`; lazy, CPU, word-list fallback; Hindi only), or `google` (opt-in — googletrans `detect()` identifies the language, enabling **Kannada/Telugu**; set `TRANSLITERATION_LANGUAGES=hi,kn,te`; a network call per query). Detection returns the *target language* (`detect_target_language`) and `RetrievalService` transliterates to that script. kn/te are validated (Wikipedia-derived eval, `scripts/build_indic_romanized_eval.py`) but opt-in — the default stays Hindi/word-list, no network.
